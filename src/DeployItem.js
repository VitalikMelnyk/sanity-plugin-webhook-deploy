...
const DeployItem = ({hook}) => {
  ...
  const confirmDialog = isConfirmOpen && (
    <Dialog width={1} header={`Delete ${hook.title}`} onClose={() => setConfirmOpen(false)}>
      <Box padding={4} paddingTop={0}>
        <Box marginBottom={4}>
          <p>
            This deletes your deploy button, but will not delete the environment. This is a
            permanent action. You'll need to re-add this environment.
          </p>
        </Box>
        {isDeleting ? (
          <Spinner />
        ) : (
          <Button tone="critical" text="Permanently Delete" onClick={onDelete} />
        )}
      </Box>
    </Dialog>
  )
    ...
    <Card padding={4} radius={2} shadow={1}>
      <CardRow>
        <MetaContainer>
          ...
          <img key={lastDeploy} src={hook.statusBadgeUrl} style={{display: 'block'}} />
        </MetaContainer>
        <ActionsContainer>
          <Box>
